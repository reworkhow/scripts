...
# FInd out the animals that need to have permanent environmental effects
rm perm.eff
for i in "${array3[@]}"
do
awk '$2!="."&&$6!="."{print $6}' phe100.$i".fill.ped" | sort -u >> perm.eff
done
mv perm.eff perm.eff.tmp
sort -u perm.eff.tmp >perm.eff

for i in "${array3[@]}"
do
cgen_z -d mat.dat.$i -e perm.eff -r y.$i -o P.$i &
done
wait

rm cg1.dat.* id.dat.* mat.dat.* perm.eff.* 
...
# Now work more on the mapfile
paste -d " " lhs.map.X lhs.map.Z lhs.map.M lhs.map.P >>lhs.map
rm lhs.map.*
...
sed s/${array3[i]}".Rinv."${array3[j]}/${array3[i]}".Rinv."${array3[j]}".v"/g lhs.map > lhs.map.temp
mv lhs.map.temp lhs.map 

done
done

idpermnum=`wc perm.eff|awk '{print $1}'`  
ident $idpermnum  > myIdent
for i in "${array2[@]}"
do
for j in "${array2[@]}"
do
...
pcgmgpu -A lhs.map -x $SOLUTION"/solve_13trait_2.sln" -b rhs.map > $SOLUTION"/solve_13trait_2.log"
